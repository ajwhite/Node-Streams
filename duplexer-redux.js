...
module.exports = function (counter) {
  ...
  t = through(function (buf) {
    if (buf.country in trackingObject) {
      trackingObject[buf.country]++;
    } else {
      trackingObject[buf.country] = 1;
    }
  }, function () {
    counter.setCounts(trackingObject);
  });

  return duplexer(t, counter);
};
